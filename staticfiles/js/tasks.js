...
function loadTasks() {
    ...
    fetch('/api/tasks/')
        ...
        .then(data => {
            ...
            tbody.innerHTML = ''; // Clear including spinner
            if (tasks.length === 0) {
                ...
                tasks.forEach(task => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <button class="btn btn-sm btn-outline-warning me-1" onclick="openTaskModal(${task.id})">Edit</button>
        ...
        .catch(error => {
            ...
            document.getElementById('taskTableBody').innerHTML = '<tr><td colspan="4" class="text-center text-muted">Error loading tasks.</td></tr>';
        });
}
    ...
    if (calendarEl) {
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick: function(info) {
                ...
                document.getElementById('eventDueDate').textContent = formatDate(info.event.start);
    ...
    if (taskId) {
        ...
        fetch(`/api/tasks/${taskId}/`)
            ...
            .then(task => {
                ...
                dueDate.value = task.due_date ? new Date(task.due_date).toISOString().slice(0, 16) : '';
